...
class RealRequest {
     ...
     * 上传文件
     */
    RealResponse uploadFile(String requestURL, File file,List<File> fileList,Map<String,File> fileMap,String fileKey,String fileType,Map<String,String> paramsMap,Map<String, String> headerMap,HiCallback callBack) {
        HttpURLConnection conn = null;
        try {
            conn = getHttpURLConnection(requestURL,"POST");
            setConnection(conn);
     ...
     * 上传文件时写文件
     */
    private void writeFile(File file, String fileKey, String fileType, DataOutputStream outputStream, final HiCallback callBack) throws IOException {
        ...
        while ((length = inputStream.read(buffer)) != -1){
            ...
            if(callBack != null){
                ...
                HiCallback.mMainHandler.post(new Runnable() {
                    @Override
                    public void run() {
